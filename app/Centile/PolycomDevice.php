...
class PolycomDevice extends Device implements DeviceManufacturerAPI
{
    public function registerOnManufacturerAPI($secret = null)
    {
        $client = new HttpClient;
        try {
            $response = $client->post(config('app.devices.manufacturer_api_url.polycom'), [
                'form_params' => [
    ...
    public function unregisterFromManufacturerAPI()
    {
        $client = new HttpClient;
        try {
            $response = $client->delete(config('app.devices.manufacturer_api_url.polycom'), [
                'query' => [
                    'mac' => macAddress($this->physicalID),
                ],
            ]);
    ...
    public function getStatusFromManufacturerAPI()
    {
      $client = new HttpClient;
      try {
          $response = $client->get(config('app.devices.manufacturer_api_url.polycom'), [
              'query' => [
                  'mac' => macAddress($this->physicalID),
              ],
          ]);
